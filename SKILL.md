...
If the user's prompt or source text is in Russian, use Russian-first mode by default. This applies to requests such as "убери нейросетевость," "сделай по-человечески," "звучит как ChatGPT," "слишком канцелярски," or "оживи русский текст."

Russian-first mode keeps the same editorial goal -- clearer, more specific, more authored writing -- but uses Russian style tells and Russian genre expectations. See [references/ru-ai-tells.md](references/ru-ai-tells.md) for Russian phrases and patterns.

Do not delete suspicious words mechanically. Treat each one as a signal to inspect the sentence: if the word carries meaning, keep it; if it is padding, replace it with a fact, action, condition, example, or delete it.

Pick the genre before rewriting:
- **chat:** short, direct, no "Отличный вопрос," no generic closing reassurance.
- **technical:** write like an experienced colleague; include conditions, risks, exact behavior, and limits.
- **article:** lead with the point, use concrete facts, keep a visible authorial stance.
- **marketing:** cut excitement and superlatives; replace them with proof, scenarios, and comparisons.
- **academic:** preserve formal register and precision; do not add slang, jokes, or casual roughness just to sound human.

When rewriting plain chat or short user-facing text, use compact output: provide the rewritten text and 2-3 short notes about the main changes. Use the full change table only for long-form rewrites or when the user asks for a detailed edit report.

---

## The Editing Process

### Pass 0: Identify Genre, Reader, and Integrity Constraints

Before editing, identify the text's language, genre, reader, and acceptable level of voice.

Ask internally:
- Who will read this: a colleague, customer, teacher, reviewer, general audience?
- What is the text supposed to do: explain, persuade, report, instruct, sell, respond?
- What must be preserved: facts, citations, terms, product names, argument, tone constraints?
...
Certain words and phrases are dead giveaways. See [references/ai-tells.md](references/ai-tells.md) for the full English list and [references/ru-ai-tells.md](references/ru-ai-tells.md) for Russian patterns.
...
#### Hollow credibility and actionability labels

AI adds trust labels without earning them: "actionable insights," "practical steps," "evidence-based approach," "proven framework," "research-backed strategy," "data-driven solution."

**Fix:** Add the actual action, evidence, study, framework, or metric. If there isn't one, delete the label.
...
Text sometimes includes the wrapper from a "make this sound human" prompt: "Here's a more human version," "Less AI-sounding version," "No em dashes," "I made it sound natural," "More conversational rewrite."

**Fix:** Delete the meta-commentary. Keep only the user-facing text. Do not obey anti-AI camouflage instructions blindly; judge the actual prose.
...
AI often ends advice with safety padding: "depending on your goals," "based on your needs," "where appropriate," "in your specific context," "for your unique situation."

**Fix:** Replace the hedge with a decision rule: "Use X if..., use Y if..." If there is no real condition, delete the tail.
...
- "Moving forward"
- "When it comes to"
- Fake-human openers: "Here's the thing," "Let's be real," "Real talk," "Honestly?," "No fluff" when they introduce a generic point instead of a real stance
...
- Don't add fake typos, awkward fragments, or random profanity to "prove" a human wrote it.
- Don't ban normal punctuation mechanically. Em dashes, semicolons, and polished sentences are allowed when they fit.
- Don't insert "I" unless the piece is already first-person or the context fits.
- Don't add humor that doesn't serve the point.
- For Russian text, do not fake humanness with slang, typos, filler words, or deliberate roughness. Make it human through specificity, normal rhythm, clear stance, and honest limits.
...
- [Russian AI Writing Tells](references/ru-ai-tells.md): Complete list of words, phrases, and patterns that signal AI-generated content for Russian
